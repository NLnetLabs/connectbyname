	...
	void *user_ref, void **refp)
{
	...
	fprintf(stderr, "connectbyname_asyn: scheme %s\n", scheme);
	if (scheme)
	{
	...
	getdns_transaction_t transaction_id)
{
	...
	case GETDNS_CALLBACK_TIMEOUT:
		...
		if (transaction_id == ctxp->trans_id_ipv4)
		{
			...
			default:
				fprintf(stderr,
			"%s, %d, dns_callback: unknown state %d (IPv4)\n",
		...
		else if (transaction_id == ctxp->trans_id_ipv6)
		{
			...
			default:
				fprintf(stderr,
			"%s, %d, dns_callback: unknown state %d (IPv6)\n",
					__FILE__, __LINE__, ctxp->state);
				abort();
			}
		}
...
static void restart_svcb(struct work_ctx *ctxp)
{
	...
	if (svcbp->ech_len)
	{
		ctxp->ech= svcbp->ech; svcbp->ech= NULL;
		ctxp->ech_len= svcbp->ech_len; svcbp->ech_len= 0;
	}
	...
	getdns_transaction_t transaction_id)
{
	...
	struct bufferevent *bev;
	struct cbn_error *errorp;
	uint8_t *alpn, *ech, *param, *svcbbuf;
	char *dname;
	ldns_buffer *buffer;
	struct svcb *svcbp;
	char addrstr[INET6_ADDRSTRLEN];
...
bad_rr:
			...
			svcbp->ech= ech; ech= NULL;
			svcbp->ech_len= ech_len;
			ctxp->nsvcb++;
	...
	{
		fprintf(stderr,
"%d: prio %d, target %s, alpn_len %lu, ech_len %lu, nipv4 %lu, nipv6 %lu\n",
			i, svcbp->prio, svcbp->target, svcbp->alpn_len,
			svcbp->ech_len, svcbp->nipv4, svcbp->nipv6);
...
static void connect_callback(evutil_socket_t fd, short events, void *ref)
{
	...
	if (ctxp->ech_len && 1)
	{
		if (!SSL_ech_add(tls, ECH_FMT_BIN, ctxp->ech_len, ctxp->ech, 
			&num_ech))
		{
			fprintf(stderr,
				"connect_callback: SSL_ech_add failed\n");
			abort();
		}
		fprintf(stderr, "connect_callback: added %d ECH keys\n",
			num_ech);
#if 0
		if (!SSL_ech_server_name(tls, ctxp->orig_hostname, "cover.defo.ie"))
		{
			fprintf(stderr,
				"connect_callback: SSL_ech_server_name failed\n");
			abort();
		}
#endif
	}

	// SSL_set_msg_callback(tls,SSL_trace); SSL_set_msg_callback_arg(tls,BIO_new_fp(stdout,0));
